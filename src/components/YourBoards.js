...
const sharedBoardsRef = firebase.database().ref('/users/'+userId+'/savedBoards');
const gradientClasses = ['red', 'green', 'blue', 'yellow', 'orange', 'pink','lightblue', 'gray'];
const byPropKey = (propertyName, value) => () => ({
    [propertyName]: value
})
const RandomGradients = ['linear-gradient(to left, #00f260, #12c2e9)', 'linear-gradient(to right, #0099f7, #f11712)', 'linear-gradient(to right, #f12711, #f5af19)', 'linear-gradient(to right, #fc00ff, #00dbde)', 'linear-gradient(to right, #ee0979, #ff6a00)'];
...
export default class YourBoards extends Component{
    ...
    orderBoardsByRecent(){
        let Ids = [];
        let Boards = this.state.ListOfBoards;
        firebase.database().ref('/users/'+userId+'/lastEdit').once('value', snapshot => {
    ...
    reorderListOfBoards(searchValue){
        ...
        this.orderBoardsByRecent();
            ...
            this.setState({ListOfBoards: _.union(fuse.search(searchValue) , Boards)}); //return the search plus the Boards, less the duplicates
        }
    ...
    selectRightPicture(boardId, t2 = 20, t3 = 100){
        let res = 0;
         boardsRef.child(boardId).child('notes').once('value', snapshot =>{
             const notes = snapshot.numChildren() || 0;
             if(notes <= t2)
                res =  'littleNotes.png';
             else if(notes > t2 && notes <= t3)
                res =  'middleNotes.png';
             else if(notes > t3)
                res = 'lotNotes.png';
         })
        ...
        sharedBoardsRef.once('value', snap => {
                ...
                boardsRef.on('child_added', snapshot => {
                    ...
                    if(snapshot.val().creator === userId){
                        firebase.database().ref('users/'+snapshot.val().creator+'/username').once('value', user => {
                            previousBoards.push({
                                id: snapshot.key,
        ...
        let boards = [];
            ...
            if(snapshot.val() !== null){
                snapshot.forEach(item => {
                    let boardObject = {id: item.key, title: item.val().title, color: item.val().color, mode:item.val().mode, grid: item.val().grid}
    ...
    render() {
        ...
        const topColor = {background: this.state.topColor};
        const isLoading = this.state.isLoading;

        const ListOfBoards = this.state.ListOfBoards.map(board =>
                <div key={board.id} id='boardLayout' className={board.color} onClick ={() => this.redirectToBoard(board.id)}>
                    <div id='board-title' >{board.title}</div>            
                    <img alt='Notes on the Board' id='boardRepresentation' src={require('./Images/'+this.selectRightPicture(board.id))}/>
                    <div id='infoBoard'>
                        <div id='creatorName'>Created by {board.creator}</div> 
                        <div id='peopleOnBoard'>People on board : {board.numberPeopleOnBoard}</div>
                    </div>
                    <img id='sharedImage' title='Shared Board' alt='Shared Board' src={require('./Images/'+board.shared)} />
                </div>
            );
              ...
              <div id='DashboardContainer'>   
                <div className='TemplateBoard'>
                    <div className='wrapper-topTemplateBoard myBoards' style={topColor} >
                        ...
                        <div id='numberOfBoards'>You currently have {numberOfBoards} boards</div>
                        <br/><button id="createButton" onClick={this.createBoard}>+</button>
                        <div id='secretColor' onClick={this.changeTopColor}></div>
                        <input type='search' onChange={e => {this.reorderListOfBoards(e.target.value); this.setState(byPropKey('searchValue', e.target.value))}} value={searchValue} placeholder='Search Boards...' onClick={e => e.target.select()} id='autoSearch'/>
                    </div>                  
                    <div className='listOfBoards-wrapper'>
