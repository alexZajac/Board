...
export default class StaticNavbar extends Component{
    ...
    componentDidMount(){
        firebase.database().ref('/users/'+this.userId).once('value', snapshot => {this.setState({username: (snapshot.val() && snapshot.val().username) || 'Anonymous'})});
    }
    ...
    render() {
        ...
        return(
          <div className="wrapper-NavbarLoggedIn">
            <img src={require('./Images/logo-full-onBlack.png')} className="LeftLogoRedirect" alt="Board"/><button className='LeftLogoRedirect' onClick={this.HomeClick} style={BtnStyle}></button>
