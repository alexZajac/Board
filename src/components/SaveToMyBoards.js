...
export default class SaveToMyBoards extends Component {
  constructor(props) {
    ...
    this.usersAllowedRef = firebase
      .database()
      .ref("/users/" + this.userId + "/savedBoards/");
  ...
  toggleMode(saved, canToggle = this.state.isVisitor) {
    if (canToggle) {
      if (saved) this.deleteBoardForUser();
      else this.saveBoardForUser();
    } else {
