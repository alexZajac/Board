...
export default class ContributionTree extends Component {
  ...
  componentDidMount(){
    ...
    boardRef.child('allowed').once('value', snapshot => {
        if(/\s/.test(snapshot.val())) //if multiple users means if white space
            this.usersAllowed = snapshot.val().split(' ');
        else    
            this.usersAllowed.push(snapshot.val());
    })
    ...
    boardRef.child('notes').once('value', snapshot => {
        ...
        this.usersAllowed.forEach(user => { //looping over each user
            //FIRST COEF
            nbOfNotes = 0;
            totalNotes = 0;
            ...
            this.boardNotes.forEach(note => {          
                ...
                { 
                    ...
                    if(note.upvotes !== null && note.upvotes !== undefined) //make sure you can convert to object
                        if(Object.values(note.upvotes).includes(user))
                            nbOfLikesOnNotes += Object.keys(note.upvotes).length-1; // number of object upvotes keys
                    else 
                        nbOfLikesOnNotes += Object.keys(note.upvotes).length;
                }
                totalNotes++;
                totalContentOfNotes += note.contents;
                totalNbOfLikes += (note.upvotes !== null && note.upvotes !== undefined) ? Object.keys(note.upvotes).length : 0;
            })
            originalNotesCoef = Math.round((nbOfNotes / totalNotes) * 100) / 100;
            originalContentCoef = Math.round((contentInNotes.length / totalContentOfNotes.length) * 100) / 100;
            originalLikeCoef = totalNbOfLikes === 0 ? 0 : Math.round((nbOfLikesOnNotes / totalNbOfLikes) * 100) / 100;
            
            const valueUser = this.createValueFromCoefs(originalNotesCoef, originalContentCoef, originalLikeCoef);
            //getting userName
            firebase.database().ref('users/'+user).once('value', snapshot => {
                 ...
                 userRef.once('value', snap => {
                    this.setState({data: previousData, username: snap.val(), isLoading: false});                                     
                }); 
  ...
  createValueFromCoefs(x,y,z){
      return 0.22 * x + 0.38 * y + 0.40 * z;
  }

  createTree(){
    const node = this.node;
    const { data, username } = this.state;
    const w = 1000, h = 700, circleWidth = 20;
    ...
    let link = myChart.selectAll('line') 
            .data(links).enter().append('line')
            .attr('stroke', 'white')
            .attr('strokewidth', '1');

    let bubble = myChart.selectAll('circle')  
            .data(data).enter() 
            .append('g') 
            .call(force.drag); 
      ...
      bubble.append('text')
            ...
            if (d.name.length <= 6){
                if(d.value < 20)
                    ...
                    return '12px';
                else if(d.value >= 30 && d.value < 50)
                    return '16px';
                else if (d.value >= 50 && d.value < 70)
                    return '22px'
                else if(d.value >= 70 && d.value < 90)
                    return '28px'
                else 
                    return '36px'
            }
  ...
  render(){
    ...
    if(redirect !== ''){
        ...
        return(
            <div>
                <img id='backToYourBoard' alt='back' title='back' src={require('./Images/backToBoards.png')} onClick={() => this.redirect()} />
                <div id='titleTree'>Contribution Tree for {boardName}</div>
                <span id='helpTreeText' style ={styleHelp}>Each circle represents a user in terms of his contribution to the Board, you are represented in white !</span>
                <img id='helpTree' src={require('./Images/helpTree.png')} alt='Help' title='Help' onClick={this.handleHelp} />
                {isLoading ? <LoaderBoard /> :
                <div className='svg-container'>
                    <svg ref={node => this.node = node} preserveAspectRatio='xMinYMin meet' viewBox='0 0 1000 800' className='svg-content-responsive'></svg>
                </div>
                }
