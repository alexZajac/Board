...
import '../medium-draft/src/index.css';
import '../medium-draft/src/components/addbutton.css';
import '../medium-draft/src/components/toolbar.css';
import '../medium-draft/src/components/blocks/text.css';
import '../medium-draft/src/components/blocks/atomic.css';
import '../medium-draft/src/components/blocks/blockquotecaption.css';
import '../medium-draft/src/components/blocks/caption.css';
import '../medium-draft/src/components/blocks/todo.css';
import '../medium-draft/src/components/blocks/image.css';
  ...
  render() {
    ...
    return (
      <button
        ...
        onClick={this.onClick}
      >
        <img id='pictureSideButtons' src={require('./Images/embed.png')} style={ImageStying}/>
      </button>
    );
  }
...
const RandomPlaceholders = ['Now tell your story...', 'This is where it all begins...','Feed me with content...','Start with a few lines...','Words matter...']
...
export default class MediumEditorCustom extends React.Component {
  constructor(props) {
    ...
    this.noteContentRef = firebase.database().ref('boards/'+this.boardId+'/notes/'+this.noteId+'/');
  ...
  fetchData() {
    ...
    this.noteContentRef.child('contents').once('value', snapshot => {
      if(snapshot.val() !== null && snapshot.val() !== '') {
        this.setState({
          editorState: EditorState.createWithContent(convertFromRaw(JSON.parse(snapshot.val())))           
          })      
      }       
  ...
  logData(e) {
    ...
    //LAST EDIT
    firebase.database().ref('/users/'+this.userId+'/lastEdit/').once('value', snapshot => {
      let hasValue = false;
      snapshot.forEach( child => {     
          if(child.val() === this.boardId && !hasValue)
          {  
             firebase.database().ref('/users/'+this.userId+'/lastEdit/'+child.key).remove();
             hasValue = true;
             firebase.database().ref('/users/'+this.userId+'/lastEdit/').push(this.boardId);
          }
      })
      if(!hasValue)
        firebase.database().ref('/users/'+this.userId+'/lastEdit/').push(this.boardId);
